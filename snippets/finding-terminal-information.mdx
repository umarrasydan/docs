---
title: "Finding Terminal Information"
description: "How to locate Terminal ID and IP address for BRI, NTT, Cashup, and Share Comm terminals"
---

## Finding Terminal Information

<Tabs>
<Tab title="BRI Terminals">

<Steps>
<Step title="Find Terminal ID (TID)">
  Open the BRI FMS app on the terminal device and locate the Terminal ID in the device information section.
  
  <Frame>
  <img src="/images/bri/bri-tid.png" alt="BRI terminal showing Terminal ID in FMS app" />
  </Frame>
</Step>

<Step title="Find IP Address">
  Open the ECRLink app on the terminal and check the network settings for the IP address.
  
  <Frame>
  <img src="/images/bri/bri-ip.png" alt="BRI terminal showing IP address in ECRLink app" />
  </Frame>
</Step>
</Steps>

</Tab>

<Tab title="NTT Terminals">

<Steps>
<Step title="Find Terminal ID (TID)">
  Look for the Terminal ID on the physical sticker attached to the device.
  
  <Frame>
  <img src="/images/ghl/ghl-tid.jpg" alt="NTT terminal showing Terminal ID sticker on device" />
  </Frame>
</Step>

<Step title="Find IP Address">
  Open the NTT LinkPOS app on the terminal and navigate to network settings to find the IP address.
  
  <Frame>
  <img src="/images/ghl/ghl-ip.jpg" alt="NTT terminal showing IP address in LinkPOS app" />
  </Frame>
</Step>
</Steps>

</Tab>

<Tab title="Cashup Terminals">

<Steps>
<Step title="Find Terminal ID (TID)">
  Open the Cashlez app and navigate to the User Account tab to locate the Terminal ID.
  
  <Frame>
  <img src="/images/cashup/cashup-tid.png" alt="Cashup terminal showing Terminal ID in User Account tab" />
  </Frame>
</Step>

<Step title="Find IP Address">
  Open the Terminal Gateway app and check the device list for the terminal IP address.
  
  <Frame>
  <img src="/images/app/gateway-app-ip-address.png" alt="Terminal Gateway app showing device IP address" />
  </Frame>
</Step>
</Steps>

</Tab>

<Tab title="Share Comm Terminals">

<Steps>
<Step title="Find Terminal ID (TID)">
  Open the Xendit Terminal app on the device and review the terminal details screen to locate the Terminal ID.
  
  <Frame>
  <img src="/images/shc/shc-tid.png" alt="Share Comm terminal showing Terminal ID in Xendit app" />
  </Frame>
</Step>

<Step title="Find IP Address">
  Open the Terminal Gateway app and check the device list for the terminal IP address.
  
  <Frame>
  <img src="/images/app/gateway-app-ip-address.png" alt="Terminal Gateway app showing device IP address" />
  </Frame>
</Step>
</Steps>

</Tab>
</Tabs>

<Note>
Screenshots and UI layouts may vary by firmware or app version. Refer to the latest vendor documentation if the interface differs from these instructions.
</Note>
