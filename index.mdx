...
- **Terminal API (H2H) key** from the Xendit In-Person Payment team (for API authentication)
- **Terminal H2H client key** from the Xendit In-Person Payment team (for SDK/app initialization)
- Terminal H2H set up (desktop app or Android/iOS SDK)
...
<Info>
**Two different keys**: The Terminal API (H2H) uses an `API_KEY` for authentication, while the Terminal H2H SDK/app uses a `CLIENT_KEY` for initialization. Contact the Xendit In-Person Payment team to obtain both keys.
</Info>

<Warning>
Terminal API (H2H) uses HTTP Basic Auth with your Terminal API (H2H) key as the username and an empty password. Always include a trailing colon when constructing the header.
</Warning>
...
1. **Create a session** with Terminal API (H2H) for an amount, currency, and terminal.
2. **Execute on device** through Terminal H2H; the customer taps/inserts/swipes.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="circle-nodes"
    href="/sdk/gateway/app-configuration"
  >
...
<CardGroup cols={2}>
  <Card
    ...
    icon="arrow-right"
    href="/guides/migration/xen-bri-to-h2h"
  >
    Migrate from Xen BRI SDK to Terminal H2H SDK with detailed code examples and architectural changes.
  </Card>
  <Card
    title="API Reference"
    icon="book"
    href="/api-reference/terminal-api/introduction"
  >
    Complete Terminal API (H2H) documentation with endpoints, authentication, and response schemas.
  </Card>
  <Card
  ...
  >
    Get personalized assistance from our Terminal API (H2H) support team for integration questions.
  </Card>
</CardGroup>
...
<Card
  ...
  href="mailto:inpersonpayments@xendit.co"
>
  Get personalized assistance from our Terminal API (H2H) support team for integration questions, device setup, or technical issues.
</Card>
