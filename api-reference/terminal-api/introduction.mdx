...
Choose your integration path based on your current setup:

<Tabs>
<Tab title="Simulation Mode (Recommended for getting started)">
  **Perfect for merchants waiting on terminal delivery**
  
  Start integrating immediately without physical hardware:
  ...
  <Step title="Create your first simulation payment">
    Use any `terminal_id` and test different scenarios with special amounts:
    ...
    const response = await fetch('https://terminal-dev.xendit.co/v1/terminal/sessions', {
      ...
      headers: {
        ...
        'Idempotency-key': 'sim-test-' + Date.now()
      },
      body: JSON.stringify({
        session_type: 'PAY',
        mode: 'TERMINAL',
        currency: 'IDR',
        amount: 10000, // Normal amount = success, 400508 = decline
        country: 'ID',
        channel_properties: {
    ...
    <Check>
    You receive a successful response and can test different scenarios using special amounts.
    </Check>
  </Step>
  
  <Step title="Test comprehensive scenarios">
    Try these special amounts to simulate different outcomes:
    - `10000`: Success flow
    ...
    <Check>
    Your error handling and webhook integration work correctly for all scenarios.
    </Check>
  </Step>
  </Steps>
  
  <Note>
  **Ready for physical terminals?** Your existing integration code will work seamlessly when you transition to physical hardware - just update the `terminal_id` values!
  </Note>
</Tab>

<Tab title="Physical Terminal Integration">
  **For merchants with terminal devices ready**
  ...
  <Step title="Create your first payment session">
    ...
    const response = await fetch('https://terminal-dev.xendit.co/v1/terminal/sessions', {
      ...
      body: JSON.stringify({
        ...
        country: 'ID',
        channel_properties: {
          terminal_id: 'TERM001' // Your actual terminal ID
        }
      })
    });
    ```
    ...
    <Check>
    Verify you receive a successful response and the physical terminal prompts for payment.
    </Check>
  </Step>
  </Steps>
</Tab>
