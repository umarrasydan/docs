...
<Info>
**New to Terminal API?** Start with our [quickstart guide](/quickstart) to get your first terminal payment processed in under 10 minutes.
</Info>
...
<Warning>
**Important**: The Terminal API requires a separate API key from your standard Xendit Dashboard credentials. Contact the Xendit In-Person Payment team to obtain your Terminal API key.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Development" icon="code">
    **Base URL**: `https://terminal-dev.xendit.co`
    
    Use this endpoint for testing and development with sandbox data.
  </Card>
  <Card title="Production" icon="globe">
    **Base URL**: `https://terminal.xendit.co`
    
    Use this endpoint for live transactions and production use.
  </Card>
</CardGroup>
...
<RequestExample>
```bash cURL
curl -X POST 'https://terminal-dev.xendit.co/api/v1/sessions' \
...
<Step title="Create your first payment session">
  ...
  ```javascript
  const response = await fetch('https://terminal-dev.xendit.co/api/v1/sessions', {
    method: 'POST',
    headers: {
...
<Card title="Push Operation Command" icon="arrow-right" href="/api-reference/terminal-api/push-operation-command">
...
<Note>
Download the complete OpenAPI 3.0 specification for integration with your development tools and API clients.
</Note>

<Card title="OpenAPI Specification" icon="file-code" href="/api-reference/terminal-api/openapi-spec.yaml">
...
## API Endpoints
- `POST /api/v1/sessions` - Create payment sessions
- `GET /api/v1/sessions/{id}` - Get session details
- `POST /api/v1/sessions/{id}/void` - Void payment transactions
- `POST /api/v1/terminals/{id}/commands` - Push terminal commands
</Update>
