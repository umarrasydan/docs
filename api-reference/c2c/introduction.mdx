...
<Tip>
**Most integrations should use the standard Terminal API (H2H)** instead of client-to-client mode. The Terminal API (H2H) provides centralized terminal management, better scalability, and simplified network requirements.
</Tip>

<Steps>
<Step title="Configure the Terminal Gateway app">
  Follow the setup in <a href="/sdk/gateway/app-configuration#client-to-client">App configuration</a> to:
...
<Step title="Use the Client-to-client API">
  After configuration, use the API documented under Terminal H2H to:
...
<Tip>
Requests must include the correct provider header (e.g., `BRI`, `NTT`, `CASHUP`) and `X-API-KEY` set in the app.
</Tip>
