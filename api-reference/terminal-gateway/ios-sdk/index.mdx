...
```swift
let result = TerminalGateway.shared.registerDevices(devices: [ 
...
<Tip>
- **BRI Terminal**: TID can be obtained from BRI FMS App, while the IP address can be found in the ECRLink app, both on the terminal device.
- **GHL Terminal**: TID can be found on the sticker on the terminal, while the IP address can be obtained from GHL LinkPOS App, both on the terminal device.
</Tip>
...
<Tab title="Restart All Connections">
  ...
  TerminalGateway.shared.restart(tid: nil)
  ```
</Tab>
...
<Tab title="Restart Single Connection">
  ...
  TerminalGateway.shared.restart(tid: TERMINAL_ID)
  ```
</Tab>
</Tabs>
...
TerminalGateway.shared
  .observeEDCConnection(terminalID: TERMINAL_ID) { state in     
