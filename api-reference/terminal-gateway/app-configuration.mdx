...
<AccordionGroup>
  <Accordion title="v1.6.0 — 5-Dec-2025">
    ...
    - Added timeout configuration for card and QR transactions (BRI only) to control operation windows
...
## Transaction Timeout (BRI only)

Use transaction timeouts to auto-cancel in the app and reissue BRI transactions when a terminal does not respond in time. You can set independent limits for card and QR flows.

<Warning>
Transaction timeout only applies to BRI. Other providers ignore these values.
</Warning>
